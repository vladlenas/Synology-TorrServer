...
make_inner_pkg() {
  ...
  cp -a ${torrserver_bin} ${tmp_dir}/bin/TorrServerMatriX
  chmod +x ${tmp_dir}/bin/TorrServerMatriX
  cp -a src/TorrServerMatriX.sc ${tmp_dir}/config/
  cp -r src/ui ${tmp_dir}
...
make_spk() {
  ...
  local spk_filename="TorrServer_MatriX-${DSM}-${TORRSERVER_VERSION}-${ARCH}.spk"
...
make_pkg() {
  ...
  echo ">>> Done"
  echo ""
  rm -rf ./build spk/TorrServer_MatriX-7.0-${TORRSERVER_VERSION}-arm5.spk
}
